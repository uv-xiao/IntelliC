...
def vector_dag(
    ...
    """Vector DAG inspired by PyPTO's intermediate orchestration example."""
...
def make_inputs(size: int = 128 * 128) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    lhs = np.linspace(-1.0, 1.0, size, dtype=np.float32)
    rhs = np.linspace(0.25, 1.25, size, dtype=np.float32)
    out = np.zeros(size, dtype=np.float32)
    summed = lhs + rhs
    expected = (summed + 1.0) * (summed + 2.0) + summed
    return lhs, rhs, out, expected


def compile_example(output_dir: Path | str) -> dict[str, Any]:
    package = compile_program(package_dir=Path(output_dir), target="pto-a2a3sim", program=vector_dag)
    return {
...
def run_package(output_dir: Path | str) -> dict[str, Any]:
    ...
    result = bind(Path(output_dir)).load(mode="sim").run("vector_dag", args=(lhs, rhs, out, lhs.size))
...
def run_demo(output_dir: Path | str) -> dict[str, Any]:
    ...
    return {
        "example": "pto.intermediate.vector_dag",
...
def main() -> None:
    print(json.dumps(run_demo(Path("artifacts") / "pto" / "intermediate" / "vector_dag"), indent=2))
